...
def render(w2c, k, timestep_data, timestep_depth_data, cfg):
    with torch.no_grad():
        ...
        (
            ...
            _,
            depth,
        ) = Renderer(raster_settings=white_bg_cam)(**timestep_data)
        (
            depth_sil,
            _,
            _,
        ) = Renderer(raster_settings=cam)(**timestep_depth_data)
...
def visualize(scene_path, cfg):
    ...
    if cfg["visualize_cams"]:
        ...
        for line_t in range(total_num_lines):
            cols.append(
                np.array(line_colormap((line_t * norm_factor / total_num_lines) + norm_factor)[:3])
            )
        cols = np.array(cols)
        all_cols = [cols]
