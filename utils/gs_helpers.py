    ...
    if i % every_i == 0 or i == 1:
        ...
        if not mapping:
            progress_bar.set_postfix(
                {
                    f"Time-Step: {iter_time_idx} | Frame {data['id']} | PSNR: {psnr:.{7}} | RMSE": f"{rmse:.{7}}"
                }
            )
            ...
            progress_bar.set_postfix(
                {
                    f"Time-Step: {online_time_idx} | Frame {data['id']} | PSNR: {psnr:.{7}} | RMSE": f"{rmse:.{7}}"
                }
            )
            progress_bar.update(every_i)
...
def eval(
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        if mapping_iters == 0 and not add_new_gaussians:
            ...
            ssim = ms_ssim(
                im.unsqueeze(0).cpu(),
                curr_data["im"].unsqueeze(0).cpu(),
                data_range=1.0,
                size_average=True,
            )
            lpips_score = loss_fn_alex(
                torch.clamp(im.unsqueeze(0), 0.0, 1.0),
                torch.clamp(curr_data["im"].unsqueeze(0), 0.0, 1.0),
            ).item()
