...
def eval_online(
    ...
    if wandb_run is not None:
        wandb_run.log(
            {
                "Final Stats/Online Average PSNR": avg_psnr,
    ...
    axs[1].set_ylabel("L1")
    fig.suptitle(
        "Average PSNR: {:.2f}, Average Depth L1: {:.2f}".format(avg_psnr, avg_l1),
        y=1.05,
        fontsize=16,
    )
...
def eval(
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        # Get current frame Gaussians
        transformed_gaussians = transform_to_frame(
            final_params, time_idx, gaussians_grad=False, camera_grad=False
        )
        ...
        if save_frames:
            ...
            cv2.imwrite(
                os.path.join(render_depth_dir, "gs_{:04d}.png".format(time_idx)), depth_colormap
            )
...
def eval_nvs(
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        if save_frames:
            ...
            cv2.imwrite(
                os.path.join(render_rgb_dir, "splatam_{:04d}.png".format(test_time_idx)),
                cv2.cvtColor(viz_render_im * 255, cv2.COLOR_RGB2BGR),
            )
            cv2.imwrite(
                os.path.join(render_depth_dir, "splatam_{:04d}.png".format(test_time_idx)),
                depth_colormap,
            )
            ...
            cv2.imwrite(
                os.path.join(rgb_dir, "gt_{:04d}.png".format(test_time_idx)),
                cv2.cvtColor(viz_gt_im * 255, cv2.COLOR_RGB2BGR),
            )
            cv2.imwrite(
                os.path.join(depth_dir, "gt_{:04d}.png".format(test_time_idx)), depth_colormap
            )
    ...
    axs[1].set_ylabel("L1 (cm)")
    fig.suptitle(
        "Average PSNR: {:.2f}, Average Depth L1: {:.2f} cm".format(avg_psnr, avg_l1 * 100),
        y=1.05,
        fontsize=16,
    )
    plt.savefig(os.path.join(eval_dir, "metrics.png"), bbox_inches="tight")
