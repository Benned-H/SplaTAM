...
from bosdyn.client.frame_helpers import get_a_tform_b, get_odom_tform_body
...
from src.spot_image_client import SpotImageClient
from src.spot_sync import SpotTimeSync
from transform_utils.kinematics import Configuration, Point3D, Pose2D, Pose3D, Quaternion
...
class SpotManager:
    ...
    def __init__(
        ...
        setup_logging(verbose=False)  # Don't use verbose logging for the Spot SDK
        ...
        self.image_client = SpotImageClient(self._robot, debug_mode=False)
    ...
    def get_tf_a_b(self, frame_a: str, frame_b: str) -> Pose3D:
        """Use the GraphNav frame snapshot to find the transform from frame_a to frame_b.

        :param frame_a: Frame transformed from
        :param frame_b: Frame transformed to
        :return: 3D pose representing pose_a_b
        """
        state = self._graph_nav_client.get_localization_state()
        tf_snapshot = state.robot_kinematics.transforms_snapshot

        tf_a_b = get_a_tform_b(tf_snapshot, frame_a, frame_b)

        return Pose3D(
            Point3D(tf_a_b.position.x, tf_a_b.position.y, tf_a_b.position.z),
            Quaternion(
                w=tf_a_b.rotation.w,
                x=tf_a_b.rotation.x,
                y=tf_a_b.rotation.y,
    ...
    def get_tf_snapshot(self) -> FrameTreeSnapshot:
        """Request the current transforms snapshot from Spot."""
        return self._graph_nav_client.get_localization_state().robot_kinematics.transforms_snapshot

    # def get_localization_state(self) -> Pose3D:
    #     """Get the current localization estimate from the GraphNav client (tf_odom_body)."""
    #     state = self._graph_nav_client.get_localization_state()
    #     self.log_info(f"Got localization: \n{state.localization}")
    #     tf_odom_body = get_odom_tform_body(state.robot_kinematics.transforms_snapshot)

    #     self.log_info(f"Got robot state in kinematic odometry frame: \n{tf_odom_body}")

    #     pos = Point3D(tf_odom_body.position.x, tf_odom_body.position.y, tf_odom_body.position.z)
    #     quat = Quaternion(
    #         x=tf_odom_body.rotation.x,
    #         y=tf_odom_body.rotation.y,
    #         z=tf_odom_body.rotation.z,
    #         w=tf_odom_body.rotation.w,
    #     )

    #     return Pose3D(pos, quat, ref_frame="odom")
